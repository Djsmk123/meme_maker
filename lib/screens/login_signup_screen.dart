...
class _LoginSignupScreenState extends State<LoginSignupScreen> {
  ...
  void initState() {
    ...
    Provider.of<AuthProvider>(context,listen: false).isPasswordResetRequest=false;
  }
  @override
  Widget build(BuildContext context) {
    bool isForgotPassword=Provider.of<AuthProvider>(context).passwordResetRequest;
    bool isLoading=Provider.of<AuthProvider>(context).loadingStatus;
    return WillPopScope(
      onWillPop: () async{
        if(isForgotPassword){
          Provider.of<AuthProvider>(context,listen:false).setPasswordResetRequest=false;
          return false;
        }
        return true;
      },
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Center(child: Image.asset("assets/images/we_go_again.png",height: 200,)),
                    ),
                    const SizedBox(height: 20,),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 20),
                      ...
                      Align(
                        ...
                        child: Padding(
                          ...
                          child: GestureDetector(onTap: () {
                            Provider.of<AuthProvider>(context,listen: false).setPasswordResetRequest=true;
                          },
                            child: Text("Forgot Password?",style: Theme.of(context).textTheme.headlineLarge!.copyWith(fontSize: 14),),
                          ...
                          if(!isForgotPassword)
                            {
                              ...
                              try{
                                await Authentication.login(email: model.email!, password: model.pass!);
                                Navigator.pop(context);
                                Navigator.push(context, MaterialPageRoute(builder: (builder)=>const TemplateSelectorScreen()));
                              }on Exception catch(e){
                                log('error $e');
                                if(e.toString()=="Exception: User with this email doesn't exist.")
                                  {
                                    try{
                                      await Authentication.signUp(email: model.email!, password: model.pass!);
                                      Navigator.pop(context);
                                      Navigator.push(context, MaterialPageRoute(builder: (builder)=>const TemplateSelectorScreen()));
                                    }catch(e){
                            ...
                            Provider.of<AuthProvider>(context,listen: false).setLoading=true;
                            try{
                              await Authentication.resetPassword(email: model.email!);
                              Fluttertoast.showToast(msg: "Link has been sent to your email address");
                              Provider.of<AuthProvider>(context,listen: false).setPasswordResetRequest=false;
                            }catch(e){
                              log(e.toString());
